...
/**  标注刷新器 */
    ...
    public void onChanged() {
        ...
        Log.i("MapAnn", "标注列表改变; 正在重新刷新...");

        AlvarMarkers           markersMsg = messageFactory.newFromType(AlvarMarkers._TYPE); //记号列表
        yocs_msgs.TableList    tablesMsg  = messageFactory.newFromType(TableList._TYPE);    //桌子列表
        yocs_msgs.ColumnList  columnsMsg  = messageFactory.newFromType(ColumnList._TYPE);   //柱子列表
...
/** 转换器 */
        ...
        for (Annotation ann: annotationsList.listFullContent()) {
            ...
            /** 添加现场列表信息 */
                ...
                Log.w("MapAnn", "不可识别的 group name: " + ann.getGroup());
            }
        }
